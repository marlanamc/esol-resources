export const questionsRealAnswersImages: Record<string, {
  url: string;
  alt: string;
  unsplashId: string;
  credit: { name: string; url: string };
}> = {
  sceneFirstDay: {
    url: "https://images.unsplash.com/photo-1523240795612-9a054b0db644?w=1200&q=80&auto=format&fit=crop",
    alt: "Adult students greeting each other and chatting before an evening class begins.",
    unsplashId: "photo-1523240795612-9a054b0db644",
    credit: { name: "Priscilla Du Preez", url: "https://unsplash.com/@priscilladupreez" },
  },
  sceneApartmentHallway: {
    url: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=1200&q=80&auto=format&fit=crop",
    alt: "Two neighbors talking in the hallway of an apartment building.",
    unsplashId: "photo-1545324418-cc1a3fa10c00",
    credit: { name: "Tierra Mallorca", url: "https://unsplash.com/@tierramallorca" },
  },
  sceneIntakeDesk: {
    url: "https://images.unsplash.com/photo-1758691737207-e75821e080cb?w=1200&q=80&auto=format&fit=crop",
    alt: "Two women talking in a modern office setting.",
    unsplashId: "photo-1758691737207-e75821e080cb",
    credit: { name: "Vitaly Gariev", url: "https://unsplash.com/@silverkblack" },
  },
  sceneWhyClassroom: {
    url: "https://plus.unsplash.com/premium_photo-1680807869114-3a7e19a81eaf?w=1200&q=80&auto=format&fit=crop",
    alt: "A group of young women standing around a classroom.",
    unsplashId: "premium_photo-1680807869114-3a7e19a81eaf",
    credit: { name: "Kübra Arslaner", url: "https://unsplash.com/@nediyodukenson" },
  },
  sceneNeighborsMeet: {
    url: "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=1200&q=80&auto=format&fit=crop",
    alt: "Two people having a friendly conversation on a city sidewalk.",
    unsplashId: "photo-1529156069898-49953e39b3ac",
    credit: { name: "Chang Duong", url: "https://unsplash.com/@chang612" },
  },
};
