...
export async function POST(request: NextRequest, { params }: Params) {
    ...
    const body = await request.json() as { submissionId?: string; voteType?: string };
    ...
    // For class votes, also prevent voting for your own group's submission
    if (voteType === "class") {
        const voterMembership = await prisma.writingGroupMember.findFirst({
            where: { studentId: userId, group: { sessionId } },
            select: { groupId: true },
        });
        if (voterMembership && submission.groupId === voterMembership.groupId) {
