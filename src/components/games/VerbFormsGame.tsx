...
export default function VerbFormsGame({ contentStr, activityId }: Props) {
  ...
  const startGame = useCallback(async () => {
    ...
    try {
        ...
        logger.debug('Fetching CSV', { csvUrl });
        const response = await fetch(csvUrl);
        const text = await response.text();
        logger.debug('CSV loaded', { length: text.length, preview: text.substring(0, 100).replace(/\n/g, '\\n') });

        const verbs = parseCSV(text);
        
        if (verbs.length === 0) {
            logger.error('No valid verbs found; check CSV format');
