import { sanitizeHtml } from "@/utils/sanitize";

interface ExampleBoxProps {
    examples: string[];
}

export function ExampleBox({ examples }: ExampleBoxProps) {
    return (
        <div className="example-box bg-bg-light border border-primary/20 rounded-lg p-4 my-4">
            <h4 className="text-sm font-semibold text-text mb-3 flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
                    />
                </svg>
                Examples:
            </h4>
            <div className="space-y-2">
                {examples.map((example, index) => (
                    <div
                        key={index}
                        className="example-item text-text bg-white px-3 py-2 rounded border-l-4 border-primary text-sm"
                        dangerouslySetInnerHTML={{ __html: sanitizeHtml(example) }}
                    />
                ))}
            </div>
        </div>
    );
}
